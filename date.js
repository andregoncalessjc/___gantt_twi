...
if (!
// Manus: Adicionado para formatar a data como YYYY-MM-DD para fácil comparação com feriados
Date.prototype.formatAsYMD = function() {
  var y = this.getFullYear();
  var m = this.getMonth() + 1; // Os meses são de 0 a 11
  var d = this.getDate();
...
// Manus: Adicionado para verificar se a data é um feriado.
// ge.master.holidays é um array de objetos Date, carregados do JSON do projeto.
Date.prototype.isHoliday = function() {
  // Verifica se o objeto global 'ge' e 'ge.master.holidays' existem e são um array.
  // 'ge' é geralmente a instância de GanttMaster.
  if (typeof ge !== 'undefined' && ge && typeof ge.master !== 'undefined' && ge.master && typeof ge.master.holidays !== 'undefined' && Array.isArray(ge.master.holidays)) {
    var currentFormattedDate = this.formatAsYMD(); // Formata a data atual como "YYYY-MM-DD"
    // Itera sobre o array de feriados (que são objetos Date)
    for (var i = 0; i < ge.master.holidays.length; i++) {
      var holidayDate = ge.master.holidays[i];
      // Verifica se o feriado é um objeto Date válido
      if (holidayDate instanceof Date && !isNaN(holidayDate)) {
        // Compara a data formatada do feriado com a data atual formatada
        if (holidayDate.formatAsYMD() === currentFormattedDate) {
          // console.log("[Manus] Data " + currentFormattedDate + " é feriado.");
          return true; // Encontrou um feriado correspondente
        }
      }
    }
  }
  // console.log("[Manus] Data " + (this.formatAsYMD()) + " NÃO é feriado.");
  return false; // Não é um feriado ou a lista de feriados não está disponível/configurada
};
